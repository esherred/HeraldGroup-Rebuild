 ...
 * @since 1.0.0
 */
function wp_aas_pro_register_post_type() {
	
	$wp_aas_pro_post_lbls = apply_filters( 'wp_aas_pro_post_labels', array(
	...
	$wp_aas_pro_slider_args = array(
		'labels'				=> $wp_aas_pro_post_lbls,
		'public'              	=> true,
		'show_ui'             	=> true,
		'query_var'           	=> false,
		'rewrite'             	=> true,
	...
	register_post_type( WP_AAS_PRO_POST_TYPE, apply_filters( 'wp_aas_pro_registered_post_type_args', $wp_aas_pro_slider_args ) );
}

// Action to register plugin post type
add_action('init', 'wp_aas_pro_register_post_type');
 ...
 * @since 1.0.0
 */
function wp_aas_pro_post_updated_messages( $messages ) {
	
	global $post, $post_ID;
	
	$messages[WP_AAS_PRO_POST_TYPE] = array(
...
add_filter( 'post_updated_messages', 'wp_aas_pro_post_updated_messages' );